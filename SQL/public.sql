...
-- Enhanced Account Management Tables
...
-- Create login history table for security page
...
CREATE INDEX IF NOT EXISTS idx_login_history_login_time ON public.login_history USING btree (login_time);
...
CREATE POLICY "Users can view own login history" ON public.login_history FOR SELECT USING (user_id = auth.uid());
