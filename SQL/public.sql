...
CREATE TABLE public.file_versions (
  ...
  CONSTRAINT file_versions_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id),
  CONSTRAINT file_versions_file_id_fkey FOREIGN KEY (file_id) REFERENCES public.drive_files(id)
);
...
CREATE TABLE public.orders (
  ...
  user_id uuid NOT NULL,
  username text,
  order_number text NOT NULL DEFAULT ((('ORD-'::text || to_char(now(), 'YYYYMMDD'::text)) || '-'::text) || substr(md5((random())::text), 1, 8)) UNIQUE,
  status text DEFAULT 'pending'::text CHECK (status = ANY (ARRAY['pending'::text, 'confirmed'::text, 'processing'::text, 'shipped'::text, 'delivered'::text, 'cancelled'::text, 'refunded'::text, 'disputed'::text])),
  ...
  currency text DEFAULT 'BDT'::text,
  payment_method text,
  payment_status text DEFAULT 'unpaid'::text CHECK (payment_status = ANY (ARRAY['unpaid'::text, 'paid'::text, 'refunded'::text, 'partial'::text])),
  ...
  CONSTRAINT orders_address_fkey FOREIGN KEY (shipping_address_id) REFERENCES public.user_addresses(id)
);
...
CREATE TABLE public.podcasts (
  ...
  CONSTRAINT podcasts_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
);
...
CREATE TABLE public.product_tag_map (
  ...
  CONSTRAINT product_tag_map_product_fkey FOREIGN KEY (product_id) REFERENCES public.products(id),
  CONSTRAINT product_tag_map_tag_fkey FOREIGN KEY (tag_id) REFERENCES public.product_tags(id)
);
...
CREATE TABLE public.products (
  ...
  status text DEFAULT 'active'::text CHECK (status = ANY (ARRAY['draft'::text, 'active'::text, 'archived'::text, 'deleted'::text])),
...
CREATE TABLE public.user_device_info (
  ...
  user_id uuid NOT NULL UNIQUE,
  username text,
  device_type text CHECK (device_type IS NULL OR (device_type = ANY (ARRAY['mobile'::text, 'tablet'::text, 'desktop'::text, 'smartwatch'::text, 'tv'::text, 'other'::text]))),
  os text,
  browser text,
  screen_size text,
  ...
  CONSTRAINT user_device_info_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
);
...
CREATE TABLE public.user_notification_channels (
  ...
  CONSTRAINT user_notification_channels_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
);
...
CREATE TABLE public.user_privacy_controls (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT user_privacy_controls_pkey PRIMARY KEY (id),
  CONSTRAINT user_privacy_controls_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
);
...
CREATE TABLE public.user_security (
  ...
  two_factor_method text CHECK (two_factor_method IS NULL OR (two_factor_method = ANY (ARRAY['totp'::text, 'sms'::text, 'email'::text, 'backup_code'::text]))),
